...
const MyTC_Info_t g_MyTC_infos[]=
{
    ...
    #ifdef TC0
        {TC0, TC0_GCLK_ID, &MCLK->APBAMASK.reg, MCLK_APBAMASK_TC0, TC0_IRQn},
    #else
        NO_TC,
    #endif

    #ifdef TC1
        {TC1, TC1_GCLK_ID, &MCLK->APBAMASK.reg, MCLK_APBAMASK_TC1, TC1_IRQn},
    #else
        NO_TC,
    #endif

    #ifdef TC2
        {TC2, TC2_GCLK_ID, &MCLK->APBBMASK.reg, MCLK_APBBMASK_TC2, TC2_IRQn},
    #else
        NO_TC,
    #endif

    #ifdef TC3
        {TC3, TC3_GCLK_ID, &MCLK->APBBMASK.reg, MCLK_APBBMASK_TC3, TC3_IRQn},
    #else
        NO_TC,
    #endif
};
